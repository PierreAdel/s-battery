...
export const fetchBatteryTimeline = cache(
  async (): Promise<ChargingState[]> => {
    "use server";
    try {
      // In a real-world scenario, we would fetch data from an API
      const response: BatteryResponse = mockData;
      return response.chargingStates;
    } catch (error) {
      console.error("Error at fetchBatteryTimeline:", error);
      throw error;
    }
  },
);
